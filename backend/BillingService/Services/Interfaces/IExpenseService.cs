...
namespace BillingService.Services.Interfaces
{
    public interface IExpenseService
    {
        ...
        Task<(ExpenseCategory? category, string? errorMessage)> CreateCategoryAsync(
        ...
        // Métodos de Despesa
        Task<(Expense? expense, string? errorMessage)> CreateExpenseAsync(
            ExpenseCreateDto dto, 
            Guid tenantId);
        Task<IEnumerable<Expense>> GetExpensesByUnidadeAsync(
            Guid unidadeId, 
            Guid tenantId);
        
        // --- 2. CORREÇÃO (Errors 1, 2, 3) ---
        // Este método estava faltando na interface
        Task<(bool success, string? errorMessage)> DeleteExpenseAsync(
            Guid expenseId, 
            Guid tenantId);
        ...
        string? errorMessage, 
        int processedRows, 
        List<int>? skippedRows)> ImportExpensesAsync(
            Stream stream, 
            string fileType, 
            Guid tenantId);
    }
